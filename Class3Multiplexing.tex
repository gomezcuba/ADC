...
\begin{itemize}
 \item For the AWGN case, the p.d.f. of $(\y|\x)$ is $\mathcal{CN}(\Hb\x,\sigma_z^2\I_{N_{r}}$)
 \begin{equation}
    \begin{split}
     \mathcal{L}_{\y}(\x)&=f(\y|\x)\\
             &=\pi^{-N_{r}}\det(\sigma_z^2\I_{N_{r}})^{-1}e^{-(\y-\Hb\x)^H(\sigma_z^2\I_{N_{r}})^{-1}(\y-\Hb\x)}\\
             &=\frac{1}{(\pi\sigma_z^2)^{N_{r}}}e^{-\frac{\|\y-\Hb\x\|^2}{\sigma_z^2}}
    \end{split}
 \end{equation}
...
%     \caption{Sphere Decoder tree}
     \begin{algorithmic}[0]
      \STATE $r\leftarrow \inf \beta_i^{\hat{\x}_{i:N_t}}\forall i\forall \hat{\x}_{i:N_t}$
      \WHILE{$\exists i>0\diagup\beta_i\leq r$}
        \FOR{each open branch}
            \IF {$\beta_i<r$}
                \STATE Expand branch
            \ENDIF
        \ENDFOR        
        \STATE $r\leftarrow \inf \beta_i\forall i$
       \ENDWHILE
     \end{algorithmic}
    \end{beamercolorbox}
 \end{column}
...
\begin{figure}
    ...
    \begin{itemize}
     ...
     \vspace{-.2in}
     $$\textnormal{SINR}^{MF}=\frac{\left|\|\h_i\|^2\right|^2\frac{P}{N_t}}{\sum_{j\neq i}|\h_i^H\h_j|^2\frac{P}{N_t}+\|\h_i\|^2\sigma^2}=\frac{\|\h_i\|^2}{\frac{\|\h_i^H\Hb\|^2-\|\h_i\|^2}{\|\h_i\|^2}+\frac{N_t\sigma^2}{P}}\;\leq \textnormal{SNR}^{MF} $$
...
\begin{figure}
    ...
    \vspace{-.1in}
    \begin{itemize}
     \item \textbf{Zero Forcing Equalizer} (decorrelator) $\W^H\Hb=\I \to\rr=\x+\W^H\z$
     \item Pseudoinverse $\W^H=(\Hb^{H}\Hb)^{-1}\Hb^H=\arg\min \Ex{}{\|\W^H\z\|^2} \textnormal{ s.t. }\W^H\Hb=\I$ 
     $$\textnormal{SNR}_{ZF}=\frac{\Ex{}{\|\x\|^2}}{\Ex{}{\|\W^H\z\|^2}}=\frac{P}{\tr\{\left(\Hb\Hb^H\right)^{-1}\}\sigma_z^2}=\frac{P}{\sigma_z^2\left(\sum_{i=1}^{N_t}\frac{1}{\lambda_1}\right)}< \textnormal{SNR}^{MF}$$
    \end{itemize}
    \pagebreak
...
\pagebreak
 \begin{itemize}
  \item Cholesky matrix inverse algorithm
    \begin{enumerate}
       \item Solve $\Lb^H\Dl\vv_{aux}=\rr_{MF}$ with $\rr_{MF}=\Hb^H\y$
...
\vspace{-.1in}
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        ...
        \node [block, right of = sum,node distance=1.5cm] (mf) {$\Hb^H$} ;
        \draw[->] (sum) -- (mf);
        \node [block, right of = mf,node distance=2cm] (lf) {$\Dl^{-1}\Lb^{-H}$} ;
...
Hermitian matrix $\M=\Lb^H\Dl\Lb$ where
$$\Lb=\left(\begin{array}{cccc}
            1&0&\dots&0\\
            \ell_{2,1}&1&\dots&0\\
            \vdots&\vdots&\ddots&\vdots\\
            \ell_{N_t,1}&\ell_{N_t,2}&\dots&1\\
\end{array}\right),\;\Dl_{N_t}=\left(\begin{array}{cccc}
 \delta_1&0&\dots&0\\
 0&\delta_2&\dots&0\\
 \vdots&\vdots&\ddots&\vdots\\
 0&0&\dots&\delta_{N_t}\\
...
\end{definition}
 \end{itemize}
    ...
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        ...
        \draw[->] (qtz) -- (out);
    \end{tikzpicture}
    \caption{Taylor series of matrix inverse $\M^{-1}=\prod_{n=1}^{\infty}(\I-\M)^n$}
    \end{figure}        
    \begin{figure}
    \centering
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        ...
        \node [pinstyle,right of = qtz,node distance=1.2cm] (out)  {$\hat{\x}$};
        \draw[->] (qtz) -- (out);
        \node [block, below of=nofilt, node distance=.8cm] (feedback) {$\I-\M$};
        \draw [->] (tap) |- node [above,pos=0.79] {} (feedback) ;
        \draw [->] (feedback) -| node[pos=0.99] {} node [near end] {} (sum2);
    \end{tikzpicture}
    \caption{Infinite Impulse Response Filter $\rr^{(n+1)}=\rr_{MF}+(\I-\M)\rr^{(n)}$}
    \end{figure}
}
...
\vspace{-.2in}
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        ...
        \node [block, right of = sum, node distance=1.5cm] (mf) {$\Hb^H$} ;
        \draw [->] (sum) -- (mf);
        \node [block, right of = mf, node distance=2cm] (ff) {$\Dl^{-1}\Lb^{-H}$} ;
...
\setbeamercolor{postit}{fg=EETblue,bg=EETblue!10}
    ...
    \pagebreak
     ...
     \item Choleski decomposition $\Hb^H\Hb=\Lb^H\Dl\Lb$ ($\Rightarrow \R=\Dl^{\frac{1}{2}}\Lb$)\\ \ \\
     \item Triangular equivalent channel
        $$\rr=\Dl^{-\frac{1}{2}}\Q^H\y=\Dl^{-\frac{1}{2}}\R\x+\Dl^{-\frac{1}{2}}\Q\z=\Lb\x+\Dl^{-\frac{1}{2}}\z'$$
    \end{itemize}
...
\vspace{-.2in}
    ...
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        ...
        \draw [->] (sum) -- (mf);
        \node [block, right of = mf, node distance=2cm] (ff) {$\Dl^{-\frac{1}{2}}$} ;
        \draw [->] (mf) -- (ff);
        \node [sum, right of=ff, node distance=2cm] (sum2) {$+$};
        \draw [->] (ff) -- node {$\rr$} (sum2);
        \node [block, right of = sum2, node distance=1.5cm] (qtz) {$\{\texttt{Q}_{\mathcal{C}}()\}^N$};
...
\vspace{-.2in}
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        ...
        \draw [->] (pf) -- (mf);
        \node [block, right of = mf, node distance=1.7cm] (ff) {$\Dl^{-\frac{1}{2}}$} ;
        \draw [->] (mf) -- (ff);
...
\begin{columns}[T]
 \begin{column}{6cm}
  \begin{itemize}
     ...
     \item $\M$ and Cholesky of ZF, MMSE, or permutations
    \end{itemize}
 \end{column}
 \begin{column}{6cm}
 \begin{table}
...
\frame[allowframebreaks]{\frametitle{\small Non-linear MIMO Receivers: Iterative Detection and Decoding}
    ...
    \pagebreak
     ...
     \begin{figure}
        \begin{tikzpicture}[scale=.5]         
                \node[draw,square] at (0,3) {\tiny $b_0$};
                \node[draw,square] at (3,3) {\tiny $b_0$};
                \node[draw,circle] at (1.5,0) {\tiny $p$};  
                \draw (0,2.5) -- (1.5,.75);
                \draw (3,2.5) -- (1.5,.75);
        \end{tikzpicture}
     \end{figure}
