...
\begin{figure}
    ...
    \begin{itemize}
     ...
     $$\textnormal{SNR}_{ZF}=\frac{\Ex{}{\|\x\|^2}}{\Ex{}{\|\W^H\z\|^2}}=\frac{P}{\tr\{\left(\Hb\Hb^H\right)^{-1}\}\sigma_z^2}=\frac{P}{\sigma_z^2\left(\sum_{i=1}^{N_t}\frac{1}{\lambda_1}\right)}< SNR^{MF}$$
    \end{itemize}
    \pagebreak
...
\vspace{-.2in}
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        ...
        \draw [->] (pf) -- (mf);
        \node [block, right of = mf, node distance=1.7cm] (ff) {$\Dl^{-\frac{1}{2}}\Lb^{-H}$} ;
        \draw [->] (mf) -- (ff);
