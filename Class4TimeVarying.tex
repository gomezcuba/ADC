...
\frame{\frametitle{MIMO Fading Channels}
    \begin{itemize}
     ...
     \item Spatial correlation $\to$ $\Sg_{\textrm{vec}\{\Hb\}}=\Ex{}{\textrm{vec}\{\Hb\}\textrm{vec}\{\Hb\}^H}\neq \I_{N_{t}N_{r}}$ 
    \end{itemize}
    \begin{definition}[Instantaneous SNR]
     $$\rho(\Hb[n])=\frac{P\|\Hb[n]\|^2}{N_{t}N_{r}\sigma_z^2}$$
    \end{definition}
    \begin{definition}[Average SNR]
     $$\overline{\rho}=\frac{P\Ex{}{\|\Hb[n]\|^2}}{N_{t}N_{r}\sigma_z^2}\stackrel{\textnormal{white}}{=}\frac{P\cancel{N_{t}N_{r}}\sigma_h^2}{\cancel{N_{t}N_{r}}\sigma_z^2}=\frac{P\sigma_h^2}{\sigma_z^2}$$
     Normalization $\Ex{}{\|\Hb[n]\|^2}=N_{t}N_{r}\to\sigma_h^2=1$
    \end{definition}
}
...
%     \end{itemize}
 ...
 \item $C$ defined $\max R$ s.t. \textbf{probability of error is as low as desired}.
    \begin{itemize}
    \item $R\leq C$ if and \textbf{only if} $\displaystyle\lim_{N_f\to\infty} P(\hat{x}\neq x) = 0 $\\ \ \\
    \end{itemize}
