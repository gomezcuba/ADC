...
\frame[allowframebreaks]{\frametitle{Fading Regimes}
 \begin{definition}[Transmitted Frame]
  Sequence of $N_f=L/M$ modulation symbols transmitted consecutively, where the transmitter sends a data packet of $L$ bits using a modulation size $M$. The transmitted signal is the sum of all finite-frame signals.
  $$x[n]=\sum_{b=-\infty}^{\infty}x[\dot{n},b]\textnormal{ where } \dot{n}=n \mod N_f \in \{0,N_f-1\}$$
 \end{definition}
 \vspace{-.1in}
 \begin{definition}[Fast Fading]
  $T_{coh}\lesssim T_s$. In each frame $N_f$ i.i.d. realizations of the stochastic process
  ...
  $T_{coh}\approx N_{f}T_s$. Channel \textbf{approximated} by frame-by-frame i.i.d. model
  $$y[\dot{n},b]\textcolor{VCobalt}{\simeq h[b]}x[\dot{n},b]+z[\dot{n},b], h[b]\textcolor{VCobalt}{\approx \textnormal{i.i.d. }\forall b}$$
  \end{definition}
 \vspace{-.1in}
 \begin{definition}[Slow Fading]$T_{coh}\gg N_fT_s$. The channel is random but remains the same in all frames
...
\frame{\frametitle{MIMO Fading Channels}
    \begin{itemize}
     ...
     \item Spatial correlation $\to$ $\K_{\textrm{vec}\{\Hb\}}=\Ex{}{\textrm{vec}\{\Hb\}\textrm{vec}\{\Hb\}^H}\neq \I_{n_tn_r}$ 
    \end{itemize}
    \begin{definition}[Instantaneous SNR]
     $$\rho(\Hb[n])=\frac{P\|\Hb[n]\|^2}{n_tn_r\sigma_z^2}$$
    \end{definition}
    \begin{definition}[Average SNR]
     $$\overline{\rho}=\frac{P\Ex{}{\|\Hb[n]\|^2}}{n_tn_r\sigma_z^2}\stackrel{\textnormal{white}}{=}\frac{P\cancel{n_tn_r}\sigma_h^2}{\cancel{n_tn_r}\sigma_z^2}=\frac{P\sigma_h^2}{\sigma_z^2}$$
     Normalization $\Ex{}{\|\Hb[n]\|^2}=n_tn_r\to\sigma_h^2=1$
    \end{definition}
}

\frame[allowframebreaks]{\frametitle{SISO Fast Fading}
    \begin{itemize}
    \item Each decoded frame experiences lots of fading realizations
        $$y[n]= h[n]x[n]+z[n]$$
...
\begin{itemize}
 ...
 \item Approximation $\overline{\mathscr{B}(\rho)}\approx\begin{cases}
...
\begin{itemize}
    \item Each frame/full system experiences \textbf{a single fading realization}
        $$y[\dot{n},b]= h[b]x[\dot{n},b]+z[\dot{n},b]$$
    \item Instantaneous SNR $\rho$ and AWGN BER $\mathscr{B}(\rho)$ (not averaged)\\ \ \\
    \item Outage probability $P_o$ $\approx$ Frame Error Rate (FER)\\ \ \\
    ...
    \begin{itemize}
        ...
        \item In Block Fading, $N_f\approx T_{coh}$ vs decoding delay
\end{itemize}
\end{itemize}
}
...
\begin{itemize}
 \item Channel used multiple times  $\{\x[n]\}_{n=0}^{N_f}$\\ \ \\
%     \begin{itemize}
%     \item Random constellation $\mathcal{C}$ with $R$ items, each $\x[n]\in\mathcal{C}$ p.d.f. $f(x)$
%     \end{itemize}
 \item {Chain rule} of M.I.  $\qquad \Inf{\{\x[n]\}_{n=0}^{N_f}}{\{\y[n]\}_{n=0}^{N_f}}\stackrel{}{=}\Inf{\x[0]}{\{\y[n]\}_{n=0}^{N_f}}+\CInf{\{\x[n]\}_{\textcolor{ARust}{n=1}}^{N_f}}{\{\y[n]\}_{n=0}^{N_f}}{\textcolor{ARust}{\x[0]}}\dots$\\ \ \\
 \item $C$ defined $\max R$ s.t. \textbf{probability of error is as low as desired}.
    \begin{itemize}
    \item $R\leq C$ if and \underline{only if} $\displaystyle\lim_{N_f\to\infty} P(\hat{x}\neq x) = 0 $\\ \ \\
    \end{itemize}
...
\end{definition}
   ...
   \begin{tikzpicture}[scale=.8]
    ...
    \node[KYJade] at (4,.6) {$\{x[n]\}_{n=0}^{N_f}$};
    \end{axis}
   \end{tikzpicture}
  \end{figure}
   ...
   \begin{tikzpicture}[scale=.8]
    ...
    \node[KYJade] at (1,.6) {$\{x[n]\}_{n=0}^{N_f}$};
    \end{axis}
   \end{tikzpicture}
  \end{figure}
    \end{itemize}
...
\begin{itemize}
 \item MRC with CSIR
    $$\rho=\frac{(|h_1|^2+|h_2|^2+\dots)P}{\sigma_z^2}\sim \frac{P}{\sigma_z^2\sqrt{2}}\chi^2(2n_r)$$
 \item Array Gain $\overline{\rho}=\frac{Pn_r}{\sigma_z^2}$
...
\begin{itemize}
 \item CSIR $\to$ 
    $$\rho=\frac{|\overset{Gaussian}{\overbrace{h_1+h_2+\dots}}|^2P}{n_t\sigma_z^2}\sim \frac{\cancel{n_t}P}{\cancel{n_t}\sigma_z^2}\textnormal{Exp}(1)$$
...
\begin{itemize}
 ...
 \item Example: Consider Rayleigh $\Hb\in\mathbb{C}^{n_t,n_r}$
    \begin{itemize}
        \item Maximum diversity gain $D=\Ex{}{\|\Hb\|^2}=n_rn_t$, \textcolor{ARust}{but 1 symbol}\\ \ \\
        \item Maximum DoF $P(\mathrm{rank}(\Hb)=\min(n_r,n_t))\approx 1$, \textcolor{ARust}{but $D=1$}\\ \ \\
    \end{itemize}
\end{itemize}
 ...
 \caption{DMT}
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
        \begin{axis}[
            ...
            xticklabels= {{$0$},{$1$},{$2$},{$\dots$},{$\min(n_t,n_r)-1$},{$\min(n_t,n_r)$}},
            ytick = {.25,2.07,4.25,6.26,16},
            yticklabels= {{$1$},{$4$},{$\vdots$},{$(n_t-1)(n_r-1)$},{$n_tn_r$}},
