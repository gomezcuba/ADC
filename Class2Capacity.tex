...
\begin{itemize}
 ...
 \item Gaussian $\y$ maximizes M.I. over all cases with $\Sb_\y=\Hb\Sb_\x\Hb^H+\Sb_\z$
 ...
 \item Gaussian $\vv$ p.d.f. $f(\vv)=\pi^{-N}\det(\Sb_\vv)^{-1}e^{-(\vv-\vec\mu)^H\Sb_\vv^{-1}(\vv-\vec\mu)}$
 $$C= \max_{\Sb_\x} \Inf{\x}{\y} \big|_{\x\sim \mathcal{CN}(0,\Sb_\x)} = \max_{\Sb_\x}\log\det \left(\I_{n_r}+\Sb_\z^{-1}\Hb\Sb_\x\Hb^H\right)$$
\end{itemize}

What about optimization constraints?
 $$C= \max_{\Sb_\x}\log\det \left(\I_{n_r}+\frac{1}{\sigma^2}\Hb\Sb_\x\Hb^H\right)\textnormal{ s.t. \textcolor{ARust}{????????}}$$
 \begin{itemize}
  \item Total average transmitted power $\tr\{\Sb_\x\}\leq P$?
  \item RF power amplifier in each antenna $x_i\leq P/n_t\forall i$?
 ...
 \begin{definition}[$C$ with only Channel State Information at the Receiver]
    $$C_{\textnormal{CSIR}}= \max_{\Sb_\x}\log\det \left(\I_{n_r}+\frac{1}{\sigma^2}\Hb\Sb_\x\Hb^H\right)\textnormal{ s.t. }\tr\{\Sb_\x\}\leq P, \Sb_x\textnormal{ const. }\forall \Hb$$
 \end{definition}
 \begin{definition}[$C$ with Channel State Information at the Transmitter]
    $$C_{\textnormal{CSIT}}= \max_{\Sb_\x}\log\det \left(\I_{n_r}+\frac{1}{\sigma^2}\Hb\Sb_\x\Hb^H\right)\textnormal{ s.t. }\tr\{\Sb_\x\}\leq P, \Sb_x=\mathscr{F}(\Hb)$$
 \end{definition}
...
\frame[allowframebreaks]{\frametitle{AWGN MIMO CSIR Capacity}
    ...
    \end{theorem}
    \item so we choose the input covariance $\Sb_\x= \frac{P}{n_t}\I_{n_t}$
    $$C_{\textnormal{CSIR}}= \log\det \left(\I_{n_r}+\frac{P}{n_t\sigma^2}\Hb\Hb^H\right)$$
    \end{itemize}
}

\frame[allowframebreaks]{\frametitle{Channel Eigenvalue Decomposition}
    \begin{itemize}
     \item $\Hb\Hb^H$ is square $n_r\times n_r$, admits \textit{eigenvalues} analysis
     \item $\Hb\Hb^H$ is Hermitian
     \begin{itemize}
        \item Non-negative eigenvalues $\lambda_1\geq \lambda_2\geq\dots\geq\lambda_{n_r}\geq 0$
        \item Decompositoin $\Hb\Hb^H=\U\LL\U^H$
        $$\U^H\U=\U^H\U=\I,\;\LL_{n_r}=\left(\begin{array}{cccc}
 \lambda_1&0&\dots&0\\
 0&\lambda_2&\dots&0\\
 \vdots&\vdots&\ddots&\vdots\\
 0&0&\dots&\lambda_{n_r}\\
\end{array}\right)$$
     \end{itemize}
    \end{itemize}
    \begin{equation*}
     \begin{split}
      C&=\log\det \left(\I_{n_r}+\frac{P}{n_t\sigma^2}\Hb\Hb^H\right)=\log\cancel{\det(\U)}^{1}\det\left(\I_{n_r}+\frac{P}{n_t\sigma^2}\LL\right)\cancel{\det(\U^H)}^{1}\\
       &=\log\prod_{j=1}^{n_r} (1+\frac{P}{n_t\sigma^2}\lambda_j)=\sum_{j=1}^{n_r}\log (1+\frac{P}{n_t\sigma^2}\lambda_j)
     \end{split}
    \end{equation*}
...
\pagebreak
     ...
     \begin{figure}
        \centering
        \begin{tikzpicture}[auto, node distance=2cm,>=latex']
            \node [pinstyle] (tx) {$x_i$ s.t. $\Ex{}{|x_i|^2}=\frac{P}{n_t}$};
            \node [output,right of=tx] (txant) {};
            \draw [-,ARust] (tx) -- (txant) \antenna;
...
\begin{itemize}
 \item Singular Value Decomposition of $\Hb=\U\Sm\V^H$
        $$\small \U^H\U=\I_{n_r},\;\V^H\V=\I_{n_t},\;\Sm=\left(\begin{array}{cccc}
 \sqrt{\lambda_1}&0&\dots&0\\
 0&\sqrt{\lambda_2}&\dots&0\\
 \vdots&\vdots&\ddots&\vdots\\
 0&0&\dots&\sqrt{\lambda_{\rank(\Hb)}}\\
\end{array}\right),\;\LL_{\rank(\Hb)}=\Sm^H\Sm=\Sm^2$$
...
\end{figure}
 \item Covariance of channel input $\Sb_\x=\V\Sb_{\x}'\V^H$
 \item $\Sb_\x$ and $\Sb_{\x}'$ square, $\V$ is unitary $\to$ bijective projection of optimal $\Sb_{\x}'$
\end{itemize}
 ...
 \begin{equation*}
  \begin{split}
    C_{\textnormal{CSIT}}%&= \max_{\Sb_\x}\log\det \left(\I_{n_r}+\frac{1}{\sigma^2}\Hb\Sb_\x\Hb^H\right)\\
                         &= \max_{\Sb_\x}\log\det \left(\I_{n_r}+\frac{1}{\sigma^2}\U\Sm\V^H\Sb_\x\V\Sm\U^H\right)\\
...
\pagebreak
\begin{itemize}
 \item System of linear equations $\vv=\Hb\x$ has solution(s) if $\rank(\Hb)\stackrel{(>)}{=} n_t$
 ...
 \end{definition}
 \begin{theorem}
  If $\Hb$ is full rank, $D=\min(n_t,n_r)$
 \end{theorem}
\end{itemize}
}
...
\begin{itemize}
 ...
 \item The DoF reveal the \textit{PRELOG} of the big-O approximation $$C\approx\Theta\left( B\min(n_t,n_r)\log\left(1+\frac{P}{N_oB}\frac{n_tn_t}{\min(n_t,n_r)}\right) \right)$$ 
 \item Spectral efficiency $\eta=\frac{C}{B}$ bits/s/Hz
 \item For small $B$, rate grows with DoF. 
    $$\eta \approx\Theta\left( \min(n_t,n_r)  \right)$$ 
 
 \pagebreak
 \item When bandwidth is very large
 $$\lim_{B\to\infty}C(bps)=\sup_{\vec{P}}B\frac{\sum_{i=1}^{\rank(\Hb)}P_i\lambda_i}{N_oB}\begin{cases}
                                                                                           \textnormal{CSIR }\to\frac{P\sum_{i=1}^{\rank(\Hb)}\lambda_i}{n_tN_oB}= \frac{P\|\Hb\|^2}{n_tN_oB}\\
...
\begin{columns}
 ...
 \begin{itemize}
    ...
    \begin{itemize}
        ...
        $$ \lim_{n_t,n_r\to\infty} C=\infty$$
        \item Random Matrix Theory
        $$\lim_{n_t\to\infty} \mathcal{CN}(0,\I_{n_t})\to U\textnormal{-sphere}$$\\ \ \\
    \end{itemize}
    \item Problems
    \begin{itemize}
